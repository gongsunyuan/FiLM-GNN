...
QoS-FiLM consists of three main components (Fig.~\ref{fig:film_gnn_arch}): \textbf{(i)} a FiLM-conditioned GNN encoder that injects per-flow QoS intent into every message-passing layer to produce differentiated topology embeddings; 
\textbf{(ii)} an actor head that pools path representations via LSTM and scores $K$ candidate paths for action selection; 
and \textbf{(iii)} a critic head that aggregates node embeddings via attention pooling for value estimation. These components operate in a closed MDP loop trained end-to-end with PPO.
...
\textbf{State} $s_t \in \mathcal{S}$ consists of three parts:
\begin{enumerate}
\item \emph{Graph snapshot}: node features (marking the source and destination $o_t,v_t$ of the current flow) and link features $[d_e,\,c_e,\,b_e^{\text{res}}]$ (delay, capacity, residual bandwidth);
...
\textbf{Reward} is taken directly from the utility function $U(\mathbf{f}_t,p_t)$ of \S\ref{sec:problem:objective} (Eq.~\eqref{eq:utility}), shifted by a constant $-1$ to narrow the reward range and facilitate value function learning:
\begin{equation}
...
\textbf{Path Pooling.} A path pooler (PathPooler) converts the node embeddings $x^{(L)}$ into candidate path representations. For the $k$-th candidate path, the node embeddings along its sequence are extracted and encoded by an LSTM to yield the path embedding $z_k\in\mathbb{R}^{h}$. The $K$ candidate paths are thus pooled into $\{z_k\}_{k=1}^{K}$.
...
The complete procedure is given in Algorithm~\ref{alg:ppo}. At each iteration, the agent collects $B$ rollout transitions by interacting with the environment using the stochastic policy $\pi_\theta$ (Step~3), then computes advantage estimates $\hat A_t$ and return targets $\hat R_t$ via GAE (Step~4).
The collected data is reused for $E$ PPO epochs with minibatch updates, optimizing the clipped surrogate loss $\mathcal{L}(\theta,\psi)$ defined in Eq.~\eqref{eq:loss} (Steps~5--11). Finally, the learning rate is decayed via cosine annealing (Step~12), and the best checkpoint is periodically saved based on deterministic evaluation rollouts (Step~13). To prevent overfitting to specific paths, link attributes and flow sequences are randomized at the start of each episode; this forces the policy to learn generalizable routing strategies rather than memorizing fixed paths.
