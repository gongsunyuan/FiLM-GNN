...
Algorithm~\ref{alg:ppo} follows the standard on-policy actor-critic pipeline: in each outer iteration, the agent collects $B$ rollout transitions by interacting with the environment using the stochastic policy $\pi_\theta$ (Lines~5--6), then computes advantage estimates $\hat A_t$ and return targets $\hat R_t$ via GAE (Lines~7--8).
The collected data is reused for $E$ PPO epochs with minibatch updates, optimizing the clipped surrogate loss $\mathcal{L}(\theta,\psi)$ defined in Eq.~\eqref{eq:loss} (Lines~9--14).
Finally, the learning rate is decayed via cosine annealing (Line~15), and the best checkpoint is periodically saved based on deterministic evaluation rollouts (Lines~16--17).
