...
第 $t$ 条流由四元组$\mathbf{f}_t = (o_t,\, v_t,\, \phi_t,\, b_t), t=1,\dots,W $描述，其中 $o_t,v_t\in V$ 为源宿节点，$b_t$ 为带宽需求，$\phi_t\in[0,1]$ 为 \emph{QoS 意图系数}。$\phi_t$ 服从 Pareto 翻转分布（参数 $\alpha$），重尾集中于低 $\phi$ 区间，使绝大部分流为时延与带宽约束严格的老鼠流，仅少量流为约束宽松的大象流。带宽需求 $b_t$ 由 $\phi_t$ 通过正相关线性映射加高斯扰动导出：
\begin{equation}
