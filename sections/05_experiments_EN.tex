...
We next compare two graph attention operators under the same FiLM framework. TransformerConv uses dot-product attention, while GATv2Conv uses dynamic additive attention~\cite{Brody2022_GATv2}, representing two different attention paradigms. Fig.~\ref{fig:conv_training} shows the training curve comparison of QoS-FiLM under the two operators (along with noFiLM-GATv2 as a reference) across three topologies; QoS-FiLM (TransformerConv + FiLM) stably outperforms FiLM-GATv2 (GATv2Conv + FiLM) on all three topologies.
