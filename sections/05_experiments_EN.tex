...
At $W\!=\!150$, QoS-FiLM outperforms all classical baselines on all three metrics of mean reward, bandwidth satisfaction, and delay (Fig.~\ref{fig:main_bars}): its mean reward of $+0.121$ surpasses the strongest classical baseline CSPF ($-0.046$) by $+0.167$, BW Score of $0.759$ exceeds CSPF ($0.473$) by $0.286$, and average delay drops from $130.0$ to $112.9$~ms. Although CSPF has the lowest delay among classical baselines ($125.9$), it does so at the sacrifice of bandwidth satisfaction (BW Score only $0.473$), and its mean reward ($-0.046$) is far below QoS-FiLM's $+0.121$; looking at delay alone cannot reflect the global trade-off in service quality. This advantage holds across all three topologies (Fig.~\ref{fig:main_bars}): QoS-FiLM outperforms the respective strongest baseline CSPF on GEANT ($+0.121$ vs.\ $-0.046$), NOBEL-EU ($-0.108$ vs.\ $-0.183$), and ABILENE ($+0.122$ vs.\ $+0.041$), demonstrating that the advantage of flow-aware representations generalizes across topologies.

\textbf{Source of Advantage: Non-Myopic Long-Range Resource Allocation.} Classical algorithms (including the ``stepwise optimal'' CSPF) make independent greedy routing decisions for each flow and lack awareness of reserving capacity for subsequent flows; under the long sequence length of $W\!=\!150$, early-arriving flows deplete critical link capacity, leaving later flows with no viable path. Bucketing by $\phi$ makes this clear: in the low-intent bucket ($\phi<0.2$, the most strictly constrained mouse flows), QoS-FiLM achieves a reward of $+0.315$, far surpassing CSPF's $+0.070$; in the mid-intent bucket the two converge; in the high-intent bucket all methods score low, an inherent resource scarcity characteristic of the task. This confirms the argument of Section~\ref{sec:problem:why_rl}: routing is a sequential decision problem with long-range credit assignment, and CSPF, as the representative of ``stepwise optimal,'' is systematically surpassed by the learned non-myopic policy.
...
We next compare two graph attention operators under the same FiLM framework. TransformerConv uses dot-product attention, while GATv2Conv uses dynamic additive attention~\cite{Brody2022_GATv2}, representing two different attention paradigms. Fig.~\ref{fig:conv_training} shows the training curve comparison of QoS-FiLM under the two operators (along with noFiLM-GATv2 as a reference) across three topologies; QoS-FiLM (TransformerConv + FiLM) consistently outperforms FiLM-GATv2 (GATv2Conv + FiLM) on all three topologies.
...
We further examine whether operator choice and conditioning interact synergistically. We supplement the noFiLM-GATv2 variant, forming a $2\times2$ comparison table of operator $\times$ conditioning (Table~\ref{tab:synergy_2x2}).

The key finding is: \emph{the gain from FiLM strongly depends on the underlying operator}. On TransformerConv, introducing FiLM brings a large improvement of $\Delta R\!=\!+0.219$ ($-0.098\!\to\!+0.121$); on GATv2, the gain from FiLM is only $\Delta R\!=\!+0.007$ ($-0.104\!\to\!-0.097$), negligible in practice. This indicates that the two dimensions are not independently additive but exhibit a strong interaction: the aforementioned difference in how link attributes are handled amplifies the gap in FiLM effectiveness. TransformerConv independently injects link attributes into attention keys and message values, providing a high-quality, link-level differentiable representational foundation for FiLM's per-layer intent modulation. In contrast, GATv2Conv mixes link attributes into node features, so even if FiLM accurately injects flow intent at each layer, the intent signal struggles to be converted into effective path differentiation through the diluted link quality representations. Therefore, the combination of TransformerConv and FiLM is not a simple superposition of two independent advantages, but a joint product in which neither component can be omitted.
...
\textbf{Structuring the Contributions: Graph Structure vs.\ FiLM.} The $2\times2$ table isolates operator$\times$conditioning, but the MLP variant (FiLM retained, message passing removed) offers a complementary perspective: by comparing MLP ($-0.177$, Table~\ref{tab:film_w150_bw}) against noFiLM-Tf ($-0.098$), we can approximately decompose the contributions of graph structure and FiLM conditioning. The transition from MLP to noFiLM-Tf (adding message passing while removing FiLM) yields $\Delta R\!=\!+0.079$, capturing the pure benefit of graph structure; the further transition to QoS-FiLM (re-introducing FiLM on the TransformerConv backbone) yields $\Delta R\!=\!+0.219$. The two components serve fundamentally different and equally indispensable roles. Graph structure supplies the spatial priors of the topology (which links exist, how congestion propagates) without which FiLM has no meaningful canvas on which to act; this is confirmed by MLP's poor performance ($-0.177$) despite retaining FiLM. FiLM conditioning, in turn, injects per-flow intent semantics that tell the graph \emph{what to prioritize}, without which the GNN treats all flows identically; this is confirmed by noFiLM-Tf's limited performance ($-0.098$). Their combination yields $+0.121$, far exceeding either component in isolation. The two are thus not competing contributors but \emph{complementary necessities}: graph structure defines the search space, and FiLM governs how that space is navigated for each flow intent.
