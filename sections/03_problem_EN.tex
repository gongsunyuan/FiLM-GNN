...
This section defines the per-flow routing problem under QoS constraints, establishes the mathematical notation used throughout the paper, presents the utility function and optimization objective, and concludes by arguing why this problem requires a reinforcement learning solution.
...
\label{eq:bw_gen}
\end{equation}
so that $\phi_t$ and $b_t$ are strongly positively correlated (Pearson $r\approx 0.90$) but not fully deterministic, i.e., $\phi_t$ still carries independent QoS information beyond $b_t$. In other words, $(\phi_t, b_t)$ jointly characterize a flow -- an elephant flow is ``large-volume and tolerant of best-effort delivery,'' while a mouse flow is ``small-volume but requires strict guarantees''; the two constitute fundamentally different QoS profiles. Flow sequences are sampled from a demand distribution $\mathcal{D}$, i.e., $(\mathbf{f}_1,\dots,\mathbf{f}_W)\sim\mathcal{D}$.
